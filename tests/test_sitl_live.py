...
def _mapped_host_for_port(port: int) -> str | None:
    ...
    try:
        result = subprocess.run(
            ["podman", "port", CONTAINER_NAME, f"{port}/tcp"],
            capture_output=True,
            text=True,
            timeout=5,
        )
...
def _port_open_in_container(
    ...
    while time.monotonic() < deadline:
        result = subprocess.run(
            ["podman", "exec", CONTAINER_NAME, "nc", "-z", "127.0.0.1", str(port)],
            capture_output=True,
            timeout=3,
        )
        if result.returncode == 0:
...
def _require_host_port(port: int) -> str:
    ...
    if host is None:
        pytest.skip("live SITL adapter tests require a container IP or mapped host port")
...
requires_live_sitl = pytest.mark.skipif(
    not _CONTAINER_OK or _CONTAINER_IP is None,
    reason=(
