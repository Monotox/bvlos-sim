...
def _wind_at(provider: LayeredWindProvider, altitude_m: float) -> tuple[float, float]:
    v = provider.wind_at(lat=0.0, lon=0.0, altitude_amsl_m=altitude_m, elapsed_time_s=0.0)
    return v.wind_east_mps, v.wind_north_mps


def test_single_layer_always_returns_that_layer() -> None:
    p = LayeredWindProvider([WindLayer(altitude_m=0.0, wind_east_mps=3.0, wind_north_mps=1.0)])
...
def test_provider_id_is_layered() -> None:
    p = LayeredWindProvider([WindLayer(altitude_m=0.0, wind_east_mps=0.0, wind_north_mps=0.0)])
