...
Use a different command prefix when needed:
    ...
    uv run python bvlos_sim/scripts/cli_batch_audit.py --command bvlos-sim
    uv run python bvlos_sim/scripts/cli_batch_audit.py --command python -m bvlos_sim
"""
...
def _write_contract_only_sitl_evidence(
    ...
    completed = subprocess.run(
        [sys.executable, "-m", "bvlos_sim", "sitl", str(scenario_path)],
        cwd=REPO_ROOT,
        text=True,
        encoding="utf-8",
        errors="replace",
